...
async function initializeApp() {
    ...
    try {
        ...
        if (!homeView) throw new Error("Critical element #home-view not found. The DOM may not be ready.");
        ...
        function showView(viewName: 'home' | 'customer' | 'admin' | 'store-info' | 'menu-detail') {
            ...
            [homeView, customerView, adminView, storeInfoView, menuDetailView].forEach(v => {
                if(v) v.classList.add('hidden')
            });
            stopCarousel();

            let viewToShow: HTMLElement | null = null;
            switch(viewName) {
        ...
        menuItemForm.addEventListener('submit', (e) => {
            ...
            const newItem: MenuItem = {
                id: id, categoryIds: selectedCategoryIds, name: adminMenuName.value,
                tags: adminMenuTags.value.split(',').map(t => t.trim()).filter(Boolean),
                description: adminMenuDescription.value, price: adminMenuPrice.value, reviews: adminMenuReviews.value || null,
                image: (adminMenuImageDropzone.querySelector('.image-preview') as HTMLImageElement).src, options: adminMenuOptions.value
            };
        ...
        detailMenuOptionsContainer.addEventListener('change', (e) => {
            ...
            if (group && group.dataset.maxSelection) {
                ...
                const checkedCount = Array.from(group.querySelectorAll<HTMLInputElement>('input[type="checkbox"]:checked')).length;
                if (checkedCount > max) { target.checked = false; return; }
            }
            ...
            checkedOptions.forEach(checkbox => { optionsPrice += parseInt(checkbox.dataset.price || '0', 10); });
        ...
        setupDragAndDrop(adminCategoryListContainer, () => appData.categories, () => { saveData(); render(); });
        ...
        document.body.innerHTML = `
            <div style="font-family: sans-serif; padding: 2em; color: #333;">
                <h2 style="color: #d9534f;">Application Error</h2>
                <p>The application failed to start. Please check the console for details or report this issue.</p>
                <pre style="background: #f5f5f5; border: 1px solid #ccc; border-radius: 4px; padding: 1em; margin-top: 1em; text-align: left; white-space: pre-wrap; word-break: break-all;">${error instanceof Error ? error.stack : String(error)}</pre>
            </div>`;
    }
}
